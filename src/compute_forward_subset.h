#ifndef __COMPUTE_FORWARD_SUBSET_H
#define __COMPUTE_FORWARD_SUBSET_H

double markov_chain::compute_forward_probabilities_subset( map<int, vector<mat> > &transition_probabilites, vector<mat> &interploidy_transitions, vector<vec> & emission_probabilities_subset ) {
    ...
    alphas[0] = emission_probabilities_subset[0] * start_prob ;
    lnl += normalize( alphas[0] ) ;
    
    /// do all other sites
    for ( int i = 1 ; i < emission_probabilities_subset.size() ; i ++ ) {
        ...
        if ( ploidy_change == true ) {
            ...
            //// otherwise, this is a transition across ploidy types on the same chromosome use the interploidy transition rates
                ...
                alphas[i] = interploidy_transitions[ploidy_switch[ploidy_index-1]-1] * alphas[i-1] % emission_probabilities_subset[i] ;
            }
        }
        
        /// otehrwise business as ususal
        else {
            alphas[i] = transition_probabilites[ploidy_switch[ploidy_index]][i] * alphas[i-1] % emission_probabilities_subset[i] ;
        }
